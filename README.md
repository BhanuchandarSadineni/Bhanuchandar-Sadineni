# Bhanuchandar-Sadineni
Profile repo of Bhanuchandar Sadineni


# Hi there, I'm Bhanuchandar Sadineni 👋

I am a **Staff Software Engineer** and Full-Stack Technologist based in the Greater Chicago Area. I specialize in designing, building, and delivering high-scale, cloud-native enterprise systems, with a deep specialization in distributed backend architectures, microservices, and financial technology platforms. 

With a results-oriented mindset and a passion for engineering modernization, I leverage technologies like Java, Kubernetes, and AWS to build resilient systems that power modern enterprise workflows.

---

### 🚀 Technical Core

*   **Backend Engineering:** Java/JEE, Spring Suite (Spring Boot, Spring Cloud, Spring Security), Microservices Architecture, API Design & Development.
*   **Cloud & Container Orchestration:** Kubernetes, , OpenShift Container Platform, Pivotal Cloud Foundry (PCF).
*   **Data & Messaging:** Enterprise Relational Databases, NoSQL, High-Throughput Message Brokers (Kafka/RabbitMQ).
*   **Domain Expertise:** Document Management Platforms, Credit Cards, Digital Banking, and Enterprise Financial Services.

---

### 💼 Professional Journey

*   **Staff Software Engineer** at **iManage** | Architecting and scaling secure cloud-native content and document management infrastructure.

---

### 🌐 Connect With Me

*   💼 **LinkedIn:** [linkedin.com/in/bhanuchandar-sadineni](https://www.linkedin.com/in/bhanuchandar-sadineni)
*   🌐 **Professional Domain:** [Coming Soon / Link Your Site Here]

